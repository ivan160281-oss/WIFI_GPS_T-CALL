...
3. **Sketch too big for the default partition scheme**: 1.70MB needed vs
   1.31MB available - the generic `esp32:esp32:esp32` board's default
   partition table leaves only a small app slot, and BLE alone typically
   adds several hundred KB. Fixed by building with
   `PartitionScheme=no_ota` (2MB app / 2MB SPIFFS, no OTA partition - this
   firmware has no OTA-update feature to lose) instead of the default -
   already set in `.github/workflows/build.yml`. This assumes the board has
   at least 4MB of flash, which is the common default for ESP32 modules
   including T-Call boards; if yours has less, `QUEUE_MAX_BYTES` in the
   `.ino` (currently ~1.5MB, comfortably inside the 2MB LittleFS partition)
   would need to shrink to match a smaller partition table.

Before flashing a whole batch of devices, specifically verify:

- **Flash size is actually 4MB or more** - `PartitionScheme=no_ota` assumes
  this; if `arduino-cli upload` (or the web flasher) reports a size mismatch,
  your board has less flash than assumed and the partition table needs
  adjusting to match.
